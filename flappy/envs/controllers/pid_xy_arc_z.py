...
class PIDController():
	def __init__(self,dt):
		...
		# gains
		self.k_1_z = 2*3			# k_1*k_s1/k_Fz = P
		self.k_s1_z = 0.16*3			# k_s1/k_Fz = D 			the *3 is to make z faster for maneuver study
		self.epsilon_z = 0.12
	...
	def get_action(self, states, dt, time, sensor_fusion):
		...
		# 	# update target
		# 	time_8 = time - 2
		# 	self.pos_target_x_ = 0.29*np.sin(2*time_8)/((np.sin(time_8))**2+1)
