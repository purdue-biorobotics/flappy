...
class Sensor_IMU_Vicon():
	def __init__(self, dt_imu, dt_vicon, states):
		...
		self.IMU_ax_noise_ = 100.0	# 1m/s^2
		self.IMU_ay_noise_ = 100.0
		self.IMU_az_noise_ = 100.0
	...
	def update_IMU(self, time, states):
		if time >= self.next_IMU_update_time_:
			...
			self.IMU_ax_ = states['body_accelerations'][0,0]*2 + np.random.normal(0,self.IMU_ax_noise_)
			self.IMU_ay_ = states['body_accelerations'][1,0]*1.5 + np.random.normal(0,self.IMU_ay_noise_)
			self.IMU_az_ = states['body_accelerations'][2,0]*1.5 + np.random.normal(0,self.IMU_az_noise_)
