...
class Simulation:
	...
	def __init__(self, mav_config_list, sim_config):
		...
		self.flapper1 = FWMAV(mav_config_list[6], self.world, self.dt_d)		#2 is latest trim with base, 4 is without base, 6 is with small base
	...
	def get_reward(self, action):
		...
		# y_projection = np.dot(j_hat,np.array([0,1,0]))
		...
		# # distance to -0.75xz plane
		# dis_xz = flapper1_states['body_positions'][2]+0.75

		# # #negative reward from y= -0.75 to y=-0.1
		# dis_xz_reward = -0.65 + dis_xz
		# if dis_xz_reward > 0:
		# 	dis_xz_reward = 0

		# #reward = 1/(cost**2+1e-6) + 4*x_projection + 1*z_projection + 12*dis_yz_reward
		# reward = 1/(cost**2+1e-6) -4*x_projection + 4*y_projection + 1*z_projection + 12*dis_xz_reward
		'''replaced with reward from v2/simulation_manuver.py'''
		reward = 0
		...
		linear_velocity = flapper1_states['body_spatial_velocities']
		angular_velocity =  flapper1_states['body_velocities'][0:3]
		print("x_angular_velocity = %.5f" % angular_velocity[0], end="\n\r")
		print("y_angular_velocity = %.5f" % angular_velocity[1], end="\n\r")
		'''TODO: negative reward for large angular velocity'''
		# roll = self.states['body_positions'][0]
		# pitch = self.states['body_positions'][1]
		# yaw = self.states['body_positions'][2]
		# x = self.states['body_positions'][3]
